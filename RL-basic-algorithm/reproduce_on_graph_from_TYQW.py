...
def main():
    sg = SupportGraph(5, [0], [4])
    sg.add_edge(0, 1, 1.0, [1])  # 添加一条边，节点0到节点1，成本为1.0，支持节点为1
    sg.add_edge(1, 2, 2.0, [2, 3])  # 添加另一条边，节点1到节点2，成本为2.0，支持节点为2和3
    sg.add_edge(2, 3, 3.0, [3])  # 添加一条边，节点2到节点3，成本为3.0，支持节点为3
    sg.add_edge(3, 4, 4.0, [4])  # 添加一条边，节点3到节点4，成本为4.0，支持节点为4

    agent_positions = [0, 1]  # 初始代理位置：代理1在节点0，代理2在节点1

    # 获取联合状态
    joint_state = sg.get_joint_state(agent_positions)

    # 执行联合动作
    actions = [(1, False), (1, True)]  # 代理1向节点1移动，不提供支持；代理2留在原地（节点1），提供支持
