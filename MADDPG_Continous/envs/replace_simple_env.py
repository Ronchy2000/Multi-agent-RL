...
class SimpleEnv(AECEnv):
        ...
        for agent in self.world.agents:
            ...
            if self.continuous_actions:
                self.action_spaces[agent.name] = spaces.Box(
                    low=-1, high=1, shape=(space_dim,) # ronchy 0 -> -1
                )
            else:
    ...
    def _execute_world_step(self):
        ...
        for i, agent in enumerate(self.world.agents):
            ...
            self._set_action(scenario_action, agent, self.action_spaces[agent.name])  #执行 n 次， 为每个agent都设置动作。 把action和agent连接

        self.world.step()  # 为每一个agent执行动作  在core.py中，底层step()函数

        global_reward = 0.0
        if self.local_ratio is not None:
            global_reward = float(self.scenario.global_reward(self.world)) # 合作奖励， 官方在simple_tag中未定义该函数
    ...
    def step(self, action):
        ...
        # print("before update-> self.current_actions[{}]={}".format(current_idx,self.current_actions[current_idx]))
        # print("action:", action)

        self.current_actions[current_idx] = action

        # print("Updated-> self.current_actions[{}]={}".format(current_idx, self.current_actions[current_idx]))
