...
class Sarsa():
    ...
    '''
    def Sarsa_alg_with_approximation(self,initial_location, epsilon = 0.1, learning_rate = 0.01):
        ...
        for episode_num in range(1000): # episode_num
            ...
            while not done:  #If s_t is not the target state, do
                ...
                q_gradient = self.gfv_a(fourier=True, state=state, action=action, ord=5)
                w = w + learning_rate * (reward + self.gama * self.qvalue[next_state, next_action] - self.qvalue[state, action]) * q_gradient
