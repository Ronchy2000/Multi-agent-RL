...
This is a predator-prey environment.

Good agents-prey (green) are faster and receive a negative reward for being hit by adversaries (red) (-10 for each collision).
Adversaries-predator are slower and are rewarded for hitting good agents (+10 for each collision).
Obstacles (large black circles) block the way.
By default, there is 1 good/prey agent, 3 adversaries/predator and 2 obstacles.

So that good-prey agents don't run to infinity, they are also penalized for exiting the area by the following function:
即prey是有边界的，超出范围有惩罚。
``` python
def bound(x):
      if x < 0.9:
